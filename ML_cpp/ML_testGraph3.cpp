...
class Node {
   public:
    ...
    /*正向传值到该神经元*/
    virtual void add_forward(double x){quitf("Empty Node, add_forward");};
    /*反向传值到该神经元*/
    virtual void add_backward(double x){quitf("Empty Node, add_backward");};
    /*从该神经元正向传值*/
    virtual void push_forward(){quitf("Empty Node, push_forward");};
    /*从该神经元反向传值*/
    virtual void push_backward(){quitf("Empty Node, push_backward");};
    /*更新权重*/
    virtual void update_w(double eta){quitf("Empty Node, update_w");};
    /*设置激活函数*/
    virtual void set_active_function(function<double(double)> F,
                                     function<double(double)> F_){quitf("Empty Node, set_active_function");};
    /*取消激活函数*/
    virtual void threshold_cancel(){quitf("Empty Node, threshold_cancel");};
};
...
class ActiveNode : public Node {
   public:
    ...
    virtual void update_w(double eta) {
        for (auto &e : pre) {
...
int main() {
    ...
    // model init
    NetworkProfile info = {(LayerProfile){784, 0, constant, constant_diff},
                           (LayerProfile){100, 1, sigmoid, sigmoid_diff},
                           (LayerProfile){10, 1, sigmoid, sigmoid_diff}};
    net.init(info);
    net.eta=0.5;
    net.prepare();
    // train
