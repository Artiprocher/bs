...
//void clear(){}
//template<class T,class... Args>void clear(T &x,Args... args){x.clear();clear(args...);}
...
public:
    ...
    /*清理*/
    void clear(){
        fill(in_val,in_val+N,0);
        fill(diff_val,diff_val+N,0);
    }
    /*更新权重*/
...
template <const int N,const int M>
void push_backward(ActiveLayer<N> &A,ActiveLayer<M> &B,ComplateEdge<N,M> &E){
    for(int j=0;j<M;j++){
        B.diff_val[j]*=B.f_(B.out_val[j]);
        for(int i=0;i<N;i++){
            A.diff_val[i]+=B.diff_val[j]*E.w[i][j];
            E.dw[i][j]=B.diff_val[j]*A.out_val[i];
        }
    }
}
